...
namespace iced_x86 {std::string_view NasmFormatter::format_register( Register reg ) const noexcept {
...
std::string NasmFormatter::format_to_string( const Instruction& instruction ) {
  std::string result;
...
void NasmFormatter::format_mnemonic( const Instruction& instruction, FormatterOutput& output ) {
  std::string_view mnemonic = get_mnemonic( instruction.mnemonic() );
  output.write_mnemonic( instruction, mnemonic );
}
void NasmFormatter::format_operands( const Instruction& instruction, FormatterOutput& output ) {
  uint32_t op_count = instruction.op_count();
...
void NasmFormatter::format_operand( const Instruction& instruction, uint32_t operand,
...
void NasmFormatter::format_register_operand( const Instruction& instruction, uint32_t operand,
...
void NasmFormatter::format_immediate( const Instruction& instruction, uint32_t operand,
                                              FormatterOutput& output ) {
  OpKind kind = instruction.op_kind( operand );
  uint64_t value = 0;
...
void NasmFormatter::format_near_branch( const Instruction& instruction, uint32_t operand,
                                                FormatterOutput& output ) {
  OpKind kind = instruction.op_kind( operand );
  uint64_t target = 0;
  int addr_size = 4;
...
void NasmFormatter::format_far_branch( const Instruction& instruction, uint32_t operand,
                                               FormatterOutput& output ) {
  (void)operand;
...
void NasmFormatter::format_memory( const Instruction& instruction, uint32_t operand,
                                           FormatterOutput& output ) {
  OpKind kind = instruction.op_kind( operand );
...
void NasmFormatter::format_number( uint64_t value, FormatterOutput& output ) {
...
void NasmFormatter::format_signed_number( int64_t value, FormatterOutput& output ) {
  if ( value < 0 ) {
    output.write( "-", FormatterTextKind::OPERATOR );
...
void NasmFormatter::write_symbol( const Instruction& instruction, FormatterOutput& output,
...
void NasmFormatter::format_evex_decorators( const Instruction& instruction, uint32_t operand,
                                                    FormatterOutput& output ) {
  (void)operand;
...
std::string_view NasmFormatter::get_mnemonic( Mnemonic mnemonic ) const {
  bool uppercase = options_.uppercase_mnemonics() || options_.uppercase_all();
  return internal::get_mnemonic_string( mnemonic, uppercase );
}
std::string_view NasmFormatter::get_memory_size_string( const Instruction& instruction ) const {
