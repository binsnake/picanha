...
namespace iced_x86 {std::string_view IntelFormatter::format_register( Register reg ) const noexcept {
...
std::string IntelFormatter::format_to_string( const Instruction& instruction ) {
  std::string result;
...
void IntelFormatter::format_mnemonic( const Instruction& instruction, FormatterOutput& output ) {
  std::string_view mnemonic = get_mnemonic( instruction.mnemonic() );
  output.write_mnemonic( instruction, mnemonic );
}
void IntelFormatter::format_operands( const Instruction& instruction, FormatterOutput& output ) {
  uint32_t op_count = instruction.op_count();
...
void IntelFormatter::format_operand( const Instruction& instruction, uint32_t operand,
  ...
  switch ( kind ) {
    case OpKind::REGISTER:
      ...
      // EVEX decorators (opmask, zeroing) apply to the first operand (destination)
      if ( operand == 0 ) {
    ...
    case OpKind::MEMORY_ESRDI:
      ...
      // EVEX decorators can also apply to memory operands when they're the destination
      if ( operand == 0 ) {
        format_evex_decorators( instruction, operand, output );
      }
      break;
...
void IntelFormatter::format_register_operand( const Instruction& instruction, uint32_t operand,
...
void IntelFormatter::format_immediate( const Instruction& instruction, uint32_t operand,
                                              FormatterOutput& output ) {
  OpKind kind = instruction.op_kind( operand );
  uint64_t value = 0;
...
void IntelFormatter::format_near_branch( const Instruction& instruction, uint32_t operand,
                                                FormatterOutput& output ) {
  OpKind kind = instruction.op_kind( operand );
  uint64_t target = 0;
  int addr_size = 4;
...
void IntelFormatter::format_far_branch( const Instruction& instruction, uint32_t operand,
                                               FormatterOutput& output ) {
  (void)operand;
...
void IntelFormatter::format_memory( const Instruction& instruction, uint32_t operand,
                                           FormatterOutput& output ) {
  OpKind kind = instruction.op_kind( operand );
  ...
  if ( kind == OpKind::MEMORY_SEG_SI || kind == OpKind::MEMORY_SEG_ESI || kind == OpKind::MEMORY_SEG_RSI ) {
    ...
    std::string_view seg_name = format_register( seg );
    output.write( seg_name, FormatterTextKind::REGISTER );
    output.write( ":", FormatterTextKind::PUNCTUATION );
    if ( kind == OpKind::MEMORY_SEG_SI ) {
      output.write( uppercase ? "[SI]" : "[si]", FormatterTextKind::TEXT );
    } else if ( kind == OpKind::MEMORY_SEG_ESI ) {
      output.write( uppercase ? "[ESI]" : "[esi]", FormatterTextKind::TEXT );
    } else {
      output.write( uppercase ? "[RSI]" : "[rsi]", FormatterTextKind::TEXT );
    }
    return;
  }
  ...
  if ( kind == OpKind::MEMORY_SEG_DI || kind == OpKind::MEMORY_SEG_EDI || kind == OpKind::MEMORY_SEG_RDI ) {
    ...
    if ( kind == OpKind::MEMORY_SEG_DI ) {
      output.write( uppercase ? "[DI]" : "[di]", FormatterTextKind::TEXT );
    } else if ( kind == OpKind::MEMORY_SEG_EDI ) {
      output.write( uppercase ? "[EDI]" : "[edi]", FormatterTextKind::TEXT );
    } else {
      output.write( uppercase ? "[RDI]" : "[rdi]", FormatterTextKind::TEXT );
    }
    return;
  }

  // Memory size prefix (e.g., "dword ptr")
  ...
  // Check for RIP-relative addressing with symbol resolution
  Register base = instruction.memory_base();
  Register index = instruction.memory_index();
  if ( symbol_resolver_ && base == Register::RIP && index == Register::NONE ) {
    // For RIP-relative, iced stores the effective address directly in displacement64
    uint64_t effective_addr = instruction.memory_displacement64();

    auto sym = symbol_resolver_->try_get_symbol( instruction, operand, operand, effective_addr, 8 );
    if ( sym ) {
      // Output as [symbol_name]
      output.write( "[", FormatterTextKind::PUNCTUATION );
      write_symbol( instruction, output, effective_addr, *sym );
      output.write( "]", FormatterTextKind::PUNCTUATION );
      return;
    }
  }
...
void IntelFormatter::format_number( uint64_t value, FormatterOutput& output ) {
...
void IntelFormatter::format_signed_number( int64_t value, FormatterOutput& output ) {
  if ( value < 0 ) {
    output.write( "-", FormatterTextKind::OPERATOR );
...
void IntelFormatter::write_symbol( const Instruction& instruction, FormatterOutput& output,
...
void IntelFormatter::format_evex_decorators( const Instruction& instruction, uint32_t operand,
                                                     FormatterOutput& output ) {
  (void)operand;
...
std::string_view IntelFormatter::get_mnemonic( Mnemonic mnemonic ) const {
  bool uppercase = options_.uppercase_mnemonics() || options_.uppercase_all();
  return internal::get_mnemonic_string( mnemonic, uppercase );
}
std::string_view IntelFormatter::get_memory_size_string( const Instruction& instruction ) const {
  bool uppercase = options_.uppercase_keywords() || options_.uppercase_all();
  MemorySize mem_size = instruction.memory_size_enum();
  return internal::get_memory_size_string( mem_size, uppercase );
}
